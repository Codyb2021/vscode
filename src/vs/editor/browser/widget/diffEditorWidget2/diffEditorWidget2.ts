...
import { appendRemoveOnDispose, applyObservableDecorations } from 'vs/editor/browser/widget/diffEditorWidget2/utils';
...
export class DiffEditorWidget2 extends Disposable implements IDiffEditor {
	...
	constructor(private readonly _domElement: HTMLElement,
		...
		this._originalEditor.onDidChangeCursorPosition(e => {
		...
		this._modifiedEditor.onDidChangeCursorPosition(e => {
			const m = this._diffModel.get();
			transaction(tx => {
		...
		const alignments = derived<IRangeAlignment[] | null>('alignments', (reader) => {
			...
			return computeRangeAlignment(this._originalEditor, this._modifiedEditor, diff.changes,
				id => alignmentViewZoneIdsOrig.has(id), id => alignmentViewZoneIdsMod.has(id));
		});
		...
		// overview ruler
		this._register(autorunWithStore2('update viewport', (reader, store) => {
